...
const settings = definePluginSettings({
    showIcon: {
        ...
        description: "Show bark button in the chat bar",
        default: true,
    },
});

const randbark = [
    "bark", "woof", "arf", "ruff", "awrf", "awruff", "grr", "boof",
    "wuff", "wruff-wruff", "woof woof", "arf arf", "bwoof", "bark bark",
    "awrf awrf", "woof-woof", "ruff ruff", "ouaf", "awrfawrf", "wooof"
];

async function handleButtonClick() {
    // @ts-expect-error typing issue
    sendMessage(getCurrentChannel().id, { content: randbark[Math.floor(Math.random() * randbark.length)] });
}

const ChatBarIcon: ChatBarButtonFactory = ({ isMainChat }) => {
    if (!isMainChat || !settings.store.showIcon) return null;
    return (
        <ChatBarButton tooltip="Bark" onClick={handleButtonClick}>
...
export default definePlugin({
    name: "Bark",
    description: "Adds a chatbar button to bark in chat",
    settings,
    requiresRestart: true,
    authors:
        [Devs.Samwich, EquicordDevs.enzomtp],
    chatBarButton: {
        icon: BarkIcon,
        render: ChatBarIcon,
    },
