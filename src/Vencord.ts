    ...
    try {
        ...
        if (Settings.autoUpdate) {
            ...
            if (Settings.autoUpdateNotification) {
                ...
                showNotice(
                    "PawsomeVencord has been updated!",
                    "Restart",
                    relaunch
                );
            }
        ...
        showNotice(
            "A new version of PawsomeVencord is available!",
            "View Update",
...
function initTrayIpc() {
    ...
    VencordNative.tray.onCheckUpdates(async () => {
        try {
            ...
            if (isOutdated) {
                showNotice("A PawsomeVencord update is available!", "View Update", () => openSettingsTabModal(UpdaterTab!));
            } else {
    ...
    VencordNative.tray.onRepair(async () => {
        try {
            ...
            UpdateLogger.error("Failed to repair PawsomeVencord", err);
        }
    });
    ...
    if (IS_DEV) {
        ...
        if (pendingPatches.length)
            PMLogger.warn(
                ...
                "You are seeing this warning because this is a Development build of PawsomeVencord.",
